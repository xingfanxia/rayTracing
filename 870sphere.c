...
void sphereInitialize(sphereSphere *sphere, double pos[3], double col[3], double r, double reflect, double refraction){
    ...
    if(refraction > 1.0)
        sphere->refraction = 1.0;
    else if (refraction < 0.0)
        sphere->refraction = 0.0;
    else
