...
void initialize(void){
    ...
    sphereInitialize(&sphereOne, position, color, 2.0, 0.0, 1.0);
    ...
    color[1] = 1.0;
    color[2] = 0.1;
    sphereInitialize(&sphereTwo, position, color, 3.0, 0.0, 1.0);
    ...
    color[1] = 1.0;
    color[2] = 1.0;
    sphereInitialize(&sphereThree, position, color, 2.0, 1.0, 0.0);
...
intersection point out into the world. */
void getRefractionRay(rayRay *ray, rayRay *rayTwo, int index, double refractionEnvironment,
    ...
    double n = refractionEnvironment / refractionMedium;
    
    double dot = vecDot(3, prevRayDir, ray->normal);
    double cTwo = sqrt((1 - (n * n)) * (1 - (dot * dot)));
    
    double temp[3];
    double tempTwo[3];
    double refractionDir[3];
    vecScale(3, n, prevRayDir, temp);
    ...
    rayInternalIntersection(&rayThree, &sphere[index]);
        //printf("didn't intersect\n");
        //printf("intersection: %f, %f, %f\n", rayThree.intersection[0], rayThree.intersection[1], rayThree.intersection[2]);
        //printf("original: %f, %f, %f\n", ray->intersection[0], ray->intersection[1], ray->intersection[2]);
    
    orig[0] = rayThree.intersection[0];
    orig[1] = rayThree.intersection[1];
    orig[2] = rayThree.intersection[2];
    prevRayDir[0] = rayThree.direction[0];
    prevRayDir[1] = rayThree.direction[1];
    prevRayDir[2] = rayThree.direction[2];
    
    n = refractionMedium / refractionMedium;
    
    dot = vecDot(3, prevRayDir, rayThree.normal);
    cTwo = sqrt((1 - (n * n)) * (1 - (dot * dot)));
    
    vecScale(3, n, prevRayDir, temp);
    vecScale(3, nTimesCMinusCTwo, rayThree.normal, tempTwo);
    vecAdd(3, temp, tempTwo, refractionDir);
    ...
    for(int i = 0; i < objectNum; i += 1){
        ...
        if(depthPotential != -1 && depthPotential < depth){
            ...
            if(sphere[i].refraction > 0.0){
                ...
                refraction = traceRay(&rayThree, 0, 10000000, refractionColor);
            }
