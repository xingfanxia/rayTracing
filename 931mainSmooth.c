...
/* compile by 
     clang 931mainSmooth.c 000pixel.o -lglfw -framework opengl;
*/
...
int traceRay(rayRay *ray, int index, int depth, double rgb[3], int bounced){
    ...
    vecAdd(3, rgbSmooth, rgb, rgbSmooth);
    for(int i = 0; i < objectNum; i += 1){
        double pointColor[3] = {0.0, 0.0, 0.0};
        double reflectionColor[3] = {0.0, 0.0, 0.0};
        double refractionColor[3] = {0.0, 0.0, 0.0};
    ...
    double temp[3] = {0, 0, 0};
    vecAdd(3, rgbSmooth, rgb, rgbSmooth);
    vecScale(3, 0.25, rgbSmooth, temp);
    vecCopy(3, temp, rgb);
    // vecCopy(3, rgbSmooth, rgb);
    return toReturn;
}
