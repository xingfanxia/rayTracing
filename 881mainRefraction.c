...
void initialize(void){
    ...
    sphereInitialize(&sphereOne, position, color, 2.0, 0.0, 1);
    ...
    color[1] = 1.0;
    color[2] = 0.0;
    sphereInitialize(&sphereTwo, position, color, 3.0, 0.0, 1);
    ...
    color[1] = 1.0;
    color[2] = 1.0;
    sphereInitialize(&sphereThree, position, color, 2.0, 1.0, 1);
...
/*Initialize a refraction ray
  Formulae refered from https://www.cs.utah.edu/~shirley/books/fcg2/rt.pdf page 14
*/
void getRefractionRay(rayRay *ray, rayRay *rayTwo, double refractionIndexBefore, double refractionIndexAfter){
    double orig[3] = {ray->intersection[0], ray->intersection[1], ray->intersection[2]};
    
    double entering[3] = {ray->direction[0], ray->direction[1], ray->direction[2]};
    
    double dot = vecDot(3, entering, ray->normal);
    if (dot <= 0) {
        return;
    }
    double temp[3];
    double temp2[3];
    double refracted[3];
    vecScale(3, dot, ray->normal, temp);
    vecSubtract(3, entering, temp, temp);
    vecScale(3, refractionIndexBefore/refractionIndexAfter, temp, temp);

    double temp3 = sqrt(1 - (1 - square(dot))*square(refractionIndexBefore/refractionIndexAfter));
    vecScale(3, temp3, ray->normal, temp2);
    vecSubtract(3, temp, temp2, refracted);

    vecUnit(3, refracted, refracted);
...
double fresnel(double leavingRefractIndex, double enteringRefractIndex, rayRay *ray, sphereSphere sphere) {
    ...
    r0 *= r0;
    double cosX = -vecDot(3, ray->normal, ray->direction);
    if (leavingRefractIndex > enteringRefractIndex) {
        double n = leavingRefractIndex/enteringRefractIndex;
    ...
    double x = 1.0 - cosX;
    double ret = r0 + (1.0 - r0)*pow(x,5);
    ret = sphere.reflection + (1.0 - sphere.reflection)*ret;
    return ret;
}
...
void combineColors(double pointCol[3], double reflectionCol[3], double refractionColor[3], sphereSphere sphere,
        int reflection, int refraction, double rgbCol[3], rayRay *ray){
    if(reflection == 1){
    ...
    // if (refraction > 0) {
    //     rgbCol[0] = pointCol[0] * sphere.refraction*refractionColor[0];
    //     rgbCol[1] = pointCol[1] * sphere.refraction*refractionColor[1];
    //     rgbCol[2] = pointCol[2] * sphere.refraction*refractionColor[2];
    // } else{
    //     rgbCol[0] = pointCol[0];
    //     rgbCol[1] = pointCol[1];
    //     rgbCol[2] = pointCol[2];
    ...
    // rgbCol[0] = pointCol[0] * (sphere.refraction*refractionColor[0] + reflectionCol[0] * sphere.reflection);
    // rgbCol[1] = pointCol[1] * (sphere.refraction*refractionColor[1] + reflectionCol[1] * sphere.reflection);
    // rgbCol[2] = pointCol[2] * (sphere.refraction*refractionColor[2] + reflectionCol[2] * sphere.reflection);
...
int traceRay(rayRay *ray, int depth, double rgb[3]){
    ...
    for(int i = 0; i < objectNum; i += 1){
        ...
        if(depthPotential != -1 && depthPotential < depth){
            ...
            } else if (sphere[i].refraction > 0.0) {
                getRefractionRay(ray, &rayThree, 1, sphere[i].refraction);
                refraction = traceRay(&rayThree, 1000000, refractionColor);
            }
            combineColors(pointColor, reflectionColor, refractionColor, sphere[i], reflection, refraction, rgb, ray);
        }
    }
    return toReturn;
}
